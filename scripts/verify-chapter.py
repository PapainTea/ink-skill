...
FORBIDDEN_PATTERNS = [
    ...
    # v0.1.18 静态陈述：身体状态字段名 + 裸数字（治本层）
    # 假定书的 quantitativeBodyState=false（大多数体裁）；数值体裁（litrpg/dungeon-core/
    # sci-fi/system-apocalypse/tower-climber）作者若要保留 HP/MP 类数字叙述，
    # 可把下 3 条从 FORBIDDEN_PATTERNS 注释掉
    (
        "身体状态字段 + 数字（如'内伤九'/'气力 7'）",
        re.compile(
            r"(内伤|旧伤|气力|魂力|灵力|精神力|伤势|疲劳|魂息)\s*"
            r"[0-9零一二三四五六七八九十]+"
        ),
    ),
    (
        ...
        re.compile(
            r"(稳在|留在|落在|处于|停在)\s*"
            r"第?\s*[一二三四五六七八九十\d]+\s*(档|级)"
        ),
    ),
    (
        "现代生理学词 + 精确数字（如'心率 84' / '血氧 96'；即使医者对白也禁）",
        re.compile(
            r"(心率|血氧|脉搏|血压|体温|呼吸频率)\s*[:：]?\s*"
            r"[0-9零一二三四五六七八九十百千]+"
        ),
    ),
]
