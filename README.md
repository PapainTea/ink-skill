...
- **v0.1.13** — **darwin 第二波（round 3+4）合并**（14 commit 压成 2 个）：7 条真 bug 治愈（reanalyze 假注释 / revise rework 步骤 / init 双版本流程致 ch7+ 丢快照 / ledger 全书种必备 / observer 单阶段修正 / 可选文件主动提示 / 枪→铁管过度审核）+ 2 次架构级文件拆分（snapshot 3-in-1 → snapshot + hook-governance + settler；audit 3-in-1 → audit + post-write-validator + writing-bans，writing-bans 被 5 模块交叉引用）+ settler 输出格式从 inkOS JSON delta 改为 skill-native Markdown（prompt 对齐 inkOS / format 可 skill-native 原则的首次落地）+ batch-write 对齐 v0.1.8/10/12（Step 8.5 Titler + 14 verify envs + SKILL_DIR）+ fanfic 写章级接入 write.md pipeline + sensitive-words 新增 v0.1.13 硬律禁 LLM 清单外自我审核（修真实事故）。新增 4 文件：hook-governance.md / settler.md / post-write-validator.md / writing-bans.md。dry_run 分 76.6 → 85 平均。
- **v0.1.12** — darwin-skill 两轮自动优化合并（安全回退点）：§8 新增 `<SKILL_DIR>` 资源路径解析（4 级 fallback：env var → `~/.claude/skills/ink/` → `~/.codex/skills/ink/` 或 `~/.gemini/extensions/skills/ink/` → 向上查找），解决跨机器跑不通的长期 bug；§2 激活自检从 5 段平铺文字重写为段首 ASCII 流程图 + 步骤 c 三分支表格（LLM 一眼可见分支逻辑），顺便修复 c.5 里 `<ink_writer 仓库>` 占位符（从未定义过）→ `<SKILL_DIR>`；§7 强制律 9 验证必贴律拆 4 个粗体子标题（规则 / 违规情形 / 违反后果 / 自检触发点）纯排版零语义改动；`scripts/bootstrap.py` Codex 安装路径 `~/.agents/` → `~/.codex/`（真 bug 修复，之前装错位置）。darwin 8 维加权分 82.8 → 86（dim 2 workflow clarity 单项 +15 权重，append-only 约束放开后解锁）。本 commit 作为安全回退点，message 以 ‼️ 标注。
